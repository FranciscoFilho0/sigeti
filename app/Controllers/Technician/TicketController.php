...
class TicketController extends Controller
{
    ...
    public function create(): void
    {
        ...
        echo $this->view->render("technician/ticket/create", [
            "title" => "Técnico | Novo Chamado - " . APP_NAME,
            "schools" => $schools
    ...
    public function edit(?array $data): void
    {
        ...
        $technicians = (new User)->where("role", "=", User::TECHNICIAN)->get();
    ...
    public function update(?array $data): void
    {
        ...
        $this->validateCsrfToken($data, "/tecnico/chamados/atualizar");

        $ticket = Ticket::find($data["id"]);

        if (!$ticket) {
            Message::warning("Chamado não encontrado ou não existe!");
            redirect("/tecnico/chamados/editar/". $data["id"]);
        }
        ...
        try {
            ...
            if (in_array($data["status"], [Ticket::FINISHED, Ticket::ARCHIVED])) {
                $ticket->setClosedAt();
            }
