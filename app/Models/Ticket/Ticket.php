...
class Ticket extends AbstractModel
{
    ...
    public function setTitle(string $title): void
    {
        ...
        if (strlen($title) > 40) {
            throw new \InvalidArgumentException("O Titulo pode ter no maximo 40 caracteres");
        }
    ...
    public function validateBusinessRules(array $data): ?array
    {
        ...
        if (!empty($data['opened_by'])) {
            $opened_by = User::find((int)$data['opened_by']);

            if (!$opened_by) {
                $errors [] = "Usuario não encontrada ou não existe.";
            } elseif ($opened_by->getRole() !== User:: TEACHER) {
                $errors[] = "O usuario selecionado não tem perfil de professor.";
            }

            $linksSchoolUser = $opened_by->schoolUserLinks();
            ...
            if (!in_array((int)$data['school_id'], $validSchoolsIds, true)) {
                $errors[] = "escola selecionada não esta vinculada ao professor selecionado";
        ...
        if (!empty($data['assigned_to'])) {
            $assigned_to = User::find((int)$data['assigned_to']);

            if (!$assigned_to) {
                $errors [] = "Usuario não encontrada ou não existe.";
            } elseif ($assigned_to->getRole() !== User:: TECHNICIAN) {
                $errors[] = "O usuario selecionado não tem perfil de tecnico.";
            }
        }
    ...
    public function validateTechnician(array $data): ?array
    {
        ...
        if (!empty($data['assigned_to'])) {
            ...
            if (!$assigned_to) {
                $errors [] = "Tecnico não encontrada ou não existe.";
            } elseif ($assigned_to->getRole() !== User:: TECHNICIAN) {
                $errors[] = "O tecnico selecionado não tem perfil de tecnico.";
            }
        }
        return $errors;
    }
    ...
    public function ticketsOrderedByStatusPriorityAndOpeningDate(): array
    {
        ...
        ORDER BY FIELD(status, 'aberto', 'em_andamento', 'aguardando', 'resolvido', 'finalizado', 'arquivado'),
    ...
    public function  countTicketsByStatus(?int $userId = null,?int $year = null): array
    {
        ...
        $sql = "select status , count(*)as quantity
                from {$this->table}
                where year(opened_at) = :year";
        ...
        foreach ($rows as $row) {
            $results[$row["status"]] = $row["quantity"];
        }
